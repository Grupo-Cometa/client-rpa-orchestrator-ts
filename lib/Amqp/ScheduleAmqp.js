...
const RabbitMqServerV2_1 = require("./RabbitMqServerV2");
class ScheduleAmqp {
    async consume() {
        await Log_1.Log.info('Aguardando 1 minutos para consumir agendamentos');
        await this.sleep(60000);
        await service.resendSchedules();
        const queue = `robot.schedules.${process.env.PUBLIC_ID}`;
        const server = new RabbitMqServerV2_1.RabbitMqServerV2(process.env.AMQP_URL);
        await Log_1.Log.success(`start consumer amqp: ${queue}`);
