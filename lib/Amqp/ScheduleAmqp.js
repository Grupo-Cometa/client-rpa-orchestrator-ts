...
class ScheduleAmqp {
    async consume() {
        ...
        await Log_1.Log.write('success', `start server.consumer`);
        await server.consume(queue, async (message) => {
            if (!message) {
                await Log_1.Log.write('info', `Message: ${JSON.stringify(message || {})}`);
                return;
            }
            const schedule = JSON.parse(message.content.toString());
            await Log_1.Log.write('success', `schedule event: ${schedule.scheduleId}`);
    ...
    async create(schedule) {
        try {
            ...
            await Log_1.Log.write('info', 'Linha 53 antes do publish');
