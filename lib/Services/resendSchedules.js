...
async function resendSchedules() {
    try {
        await Log_1.Log.write('info', `Excutando resend`);
        const robotId = await Orquestrador_1.orquestrador.getRobotId(process.env.PUBLIC_ID);
        await Log_1.Log.write('info', `Robot id: ${robotId}`);
        const response = await Orquestrador_1.orquestrador.resendSchedules(robotId);
        await Log_1.Log.write('success', `Agendamentos publicados: ${JSON.stringify(response.data?.data)}`);
    }
    catch (error) {
        await Log_1.Log.write('error', `Erro ao publicar os agendamento: ${error?.message}`);
    }
}
